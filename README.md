...
- User collection data stays in the browser by default, with browser-session restore plus optional user-controlled cross-session browser-storage persistence on the same device.
- Reference enrichment comes from static snapshots in `site/data/reference/` generated from the Skycards API.
- Completionist mode uses a shared delayed flight snapshot served from Cloudflare in production, with `site/data/runtime-config.json` selecting the active and shadow sources, `site/data/live/` kept as the local fixture path, and matching against a user's collection still happening only in that user's browser. The browser checks the shared manifest every `15` seconds and only downloads a new snapshot when the published version changes, while the UI now uses the same two-line refresh language as daily missions: `Last updated ... ago.` followed by `Next update expected in ...`.
- Daily missions uses a separate standalone page at `site/daily-missions/`, opens in `All missions` mode by default, and reads the canonical `derived/daily-missions` manifest published by `fr24-discord-bot`, with query-string support for `?date=YYYY-MM-DD`, `?mission=<mission-key>`, and `?dailyMissionsManifestUrl=<absolute-url>`. The page also checks its manifest every `15` seconds and only downloads a new board when the shared version changes, using the same two-line refresh copy as completionist mode so both live surfaces read the same way. It can also optionally load a local `skycards_user.json` in-browser and draw the same blue `100 km` / `200 km` `completedAirportIds` coverage radii used on the main completionist map.
...
The preview server now binds `localhost` by default so `localhost`, `127.0.0.1`, normal desktop browsers, and Playwright all hit the same working loopback server on Windows. If you override `--host 127.0.0.1`, open `http://127.0.0.1:4173/` explicitly instead of `localhost`.
The preview command now self-checks the advertised loopback URLs before it prints success. If it exits with a reachability error, another local process is already intercepting part of that port; stop the conflicting listener or pick a different port with `--port`.
...
When validating in Codex desktop, prefer the built-in Playwright browser tools first, then `js_repl` for repeated interactive checks once the browser is open. If terminal browser work is still needed, use the global `playwright-cli` binary and keep `npx ... @playwright/cli` as fallback-only.
...
1. A short private startup screen appears while Skyviz checks for browser-session data and any saved local data on the current device.
...
4. (Optional) Enable the local save checkbox to keep the uploaded export, including completionist hide choices, in browser storage on the current device after you close the browser.
...
9. Use the compact `Daily Missions` launch card and data-tools icon next to the tab toggle to either open the standalone mission board in a new tab or manage the active dashboard: upload a new export, turn browser-local save on or off for the current upload, or clear the current upload plus its browser-session and saved local data.
...
12. Use the `Map` tab for captured vs missing airport plotting with airport completion shown directly on the map card plus a single right-side `Airport completion explorer` drill-down panel. The drill path is `Continents -> Countries -> US states` (US states appear when `United States` is selected). Continents and the `United States` row show explicit drill cues, and the panel includes helper copy plus `Back` + breadcrumb controls so it is always clear where you are in the hierarchy. Clicking a row focuses/highlights/zooms the map for that region and advances the drill level when a deeper level exists. The widget also includes explicit sort-category selection (`% complete`, `total airports`, `captured airports`, `name`), an adjacent ascending/descending arrow toggle (default `total airports` descending), and a `Details` expander per row with captured/remaining code lists plus per-scope `Copy` / `Export` actions. Airport dots scale up as you zoom in so map targets stay easier to click. Airport popups show `ICAO (IATA)` when available, the airport name, and direct links for both `View on FR24` and `View on Skydex`. At higher zoom levels, muted airport code labels render over dots with a cap to avoid excessive overlap. A `Completionist mode` toggle swaps that drill-down view for a live-flight hunter that checks a shared delayed snapshot against your local missing airports and missing aircraft cards, ignores live feed rows that do not include a usable registration, scales aircraft markers by map zoom, pairs live sidebar search with a compact sort control, keeps only smaller `Missing airport` and `New card` pills (both enabled by default and independently toggleable), groups the live results by the busiest airport/card targets when sorting by traffic, promotes refresh status into the same two-line `Last updated ... ago.` plus `Next update expected in ...` block used on daily missions, hides the base map legend while the live-hunter view is active, and keeps map clicks and sidebar selection locked to one stable highlighted result row without runaway auto-scrolling. Completionist also resolves every uploaded `completedAirportIds` airport against the reference dataset and draws translucent blue `100 km` plus `200 km` coverage circles around each one, so overlapping coverage areas naturally read a bit darker. Completionist now checks its shared manifest every `15` seconds and only downloads a new snapshot when the published version changes, so the live map tracks new shared publishes without repeatedly pulling the same payload on a blind minute timer. Each completionist row now leads with the aircraft name plus ICAO type, keeps the age on the same line, shows the registration with optional callsign in muted text, compresses the route to IATA codes plus full airport names, can surface compact grouped-flight badges when traffic sorting is active, reuses the same aircraft marker artwork as the live map between origin and destination, and keeps the `Open in FR24` plus `Hide airport` / `Hide aircraft` actions inside the card surface for faster scanning with matched typography across those action controls. Those hide choices stay attached to the current upload, survive refreshes in the same browser session, also persist across later sessions when local save is enabled, and reset as soon as a new file is loaded. When a focused completionist flight also represents a missing airport target and the destination airport has valid reference coordinates, the map adds a temporary destination point and fits the focused view to both the aircraft and that missing airport. Completionist popups mirror that same compact aircraft-first hierarchy, and clicking the selected aircraft again or using the popup `X` clears focus cleanly. Background rerenders now preserve the current airport/completionist pan and zoom, so only explicit map-reset and focus actions move the viewport. Completionist target-pill counts use unique target counts, so repeated flights to the same missing airport or repeated sightings of the same missing aircraft card do not inflate those totals.
13. On desktop and tablet viewports, the default map view still starts with Skyviz’s responsive split behavior, and the header sliders can override map width and height while persisting that preference in the current browser. Width sliders now preview the target value while you drag and only commit the layout when you release, which keeps the page from reflowing under the pointer; height still updates live. When growing wider on roomy desktops, the map first spends available outer page gutter before it starts squeezing the adjacent explorer column. Completionist mode reuses the same width and height sliders against the wider live-flight layout, now defaults both sliders to their maximum values when it is switched on, and keeps its top map header condensed into a single-row control strip across tablet and desktop widths so more of the map stays visible by default. On tablet widths, completionist width overrides stay in the stacked layout so the header and slider tracks keep usable width instead of collapsing into a cramped split. Outside completionist mode, tablet widths still default to the clean single-column stack until the user chooses an override. Phone widths keep the controls hidden so the live-flight hunter stays usable without side-by-side overflow.
...
19. The standalone `/daily-missions/` page now opens in `All missions` mode with no standalone hero, a more visual desktop command header that carries the mission filters, a desktop map-and-rail workspace with browser-local header sliders for user-controlled framing, finder values placed above live matches in the right rail, and a mobile flow that keeps the map first while still exposing the rail immediately below for easy scrolling. Its width slider also previews the target while you drag and commits the layout on release so the control does not jump across the page mid-adjustment. On wide desktops, widening the mission map still consumes available outer page gutter before it starts shrinking the rail. Its map now uses real aircraft model silhouettes rotated to the live heading when `track` is available, keeps the pins aircraft-first without collapsing them into oversized numbered circles, pins the selected-flight telemetry popover directly to the chosen aircraft with a small `Open in FR24` link, preserves the current pan/zoom during background refreshes unless the user explicitly changes mission scope or focuses a flight, checks the shared daily-missions manifest every `15` seconds so the board can follow newly published live versions without re-downloading the same payload on a blind minute loop, uses the same two-line refresh status copy as completionist mode so both live surfaces read the same way at a glance, and includes an optional browser-local `skycards_user.json` uploader that resolves `completedAirportIds` into the same blue `100 km` / `200 km` weekly airport coverage circles used on the main completionist map.
